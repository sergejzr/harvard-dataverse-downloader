...
public class MainFrame extends JFrame {
    ...
    private static final String NON_DATAVERSE_MESSAGE_PREFIX =
    ...
    private void installDownloadStateRefresh() {
        downloadManager.addListener(task ->
                SwingUtilities.invokeLater(() -> datasetTreePanel.refreshDownloadState()));
    }

    private void installDownloadProgressRefresh() {
        downloadManager.addListener(task ->
                SwingUtilities.invokeLater(this::refreshDownloadProgressStatus));
    }
    ...
    private void refreshDatasetHistory() {
        ...
        Object currentValue = datasetField.isEditable()
                ? datasetField.getEditor().getItem()
    ...
    private void installDatasetHistoryPopupBehavior() {
        ...
        MouseAdapter adapter = new MouseAdapter() {
            ...
            private void maybeShowPopup(MouseEvent e) {
                ...
                showDatasetHistoryPopup(e.getComponent(), e.getX(), e.getY());
            }
        };
    ...
    private void showDatasetHistoryPopup(Component invoker, int x, int y) {
        List<String> history = preferencesService.loadDatasetUrlHistory(preferences.getDatasetUrlHistorySize());
        if (history.isEmpty()) {
            return;
        }
        ...
        for (String item : history) {
            ...
            menuItem.addActionListener(e -> datasetField.getEditor().setItem(item));
    ...
    private void installDatasetEditorContextMenu() {
        ...
        pasteFromClipboardAndLoadItem.addActionListener(e -> {
            try {
                ...
                JOptionPane.showMessageDialog(
                        this,
                        "Could not read clipboard: " + ex.getMessage(),
                        "Error",
                        JOptionPane.ERROR_MESSAGE);
            }
        });
